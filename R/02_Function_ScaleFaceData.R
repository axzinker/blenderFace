...
#'
#' The heads of the participants are of different sizes. This function scales and resizes the x/y values according to a standardized face.
#'
#' @param data Data frame containig the columns which should be rescaled. Remaining colums will be returned untouched (e.g., z-values because we have no information to rescale them).
#' @param colNames Character vector with names of the columns which should be rescaled. Variable names containing vertical movement should end with "_x", whereas variable names containing horizontal movement should end with "_y".
#' @param scaleFactor Numeric variable containing the value of the real world object measured in blender. For example, this can be the diameter of a glue dot measured in blender (default = 8 mm). Therefore, scaling should be done person by person.
#' @param pupilDist Single numeric value. Distance from left to right pupile measured in blender units
#' @param mouthcornerDist Single numeric value. Distance from left to right mouthcorner measured in blender units
#' @param leftPMDist Single numeric value. Distance from left pupile to left mouthcorner measured in blender units
#' @param rightPMDist Single numeric value. Distance from right pupile to right mouthcorner measured in blender units
