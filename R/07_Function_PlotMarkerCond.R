...
#' @param colNames: character vector naming the two columns with the x/y-Values to be plotted
#' @param colNamesCond: character vector naming the column containig the conditions
#' @param title: Default is no title.
#' @param verbose: print verbose output 
#'   
#' @return Plot of median movement of marker with quartile ellipse around per condition
#'
...
PlotMarkerCond<- function(data, colNames, colNameCond, title="", verbose=FALSE) {
  ...
  legend("topright", cex=.7, inset=.0, cond, col= 1:length(cond), pch=19, horiz=FALSE)
  ...
  for (j in 1:length(cond)) {
    ...
    quantX <- quantile(data[data[colNameCond] == cond[j],colNames[1]],c(.25,.75),na.rm=TRUE)
    quantY <- quantile(data[data[colNameCond] == cond[j],colNames[2]],c(.25,.75),na.rm=TRUE)
    lines(c(0,medianX),c(0,medianY),type="l",col=j)
    points(medianX,medianY, pch=19, col=j)
    draw.ellipse(medianX,medianY, a = abs(quantX[2]), b = abs(quantY[2]),segment=c(0,90),arc.only=FALSE,border=j,lty=1,lwd=1,deg=TRUE)
    draw.ellipse(medianX,medianY, a = abs(quantX[1]), b = abs(quantY[2]),segment=c(90,180),arc.only=FALSE,border=j,lty=1,lwd=1,deg=TRUE)
    draw.ellipse(medianX,medianY, a = abs(quantX[1]), b = abs(quantY[1]),segment=c(180,270),arc.only=FALSE,border=j,lty=1,lwd=1,deg=TRUE)
    draw.ellipse(medianX,medianY, a = abs(quantX[2]), b = abs(quantY[1]),segment=c(270,360),arc.only=FALSE,border=j,lty=1,lwd=1,deg=TRUE)
  }
}
