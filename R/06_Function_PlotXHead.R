...
#'
#' @param data matrix or data-frame with a row sequence of *standardized* x/y coordinates for each marker. Please use function "ScaleFaceData" to standardize data.
#' @param dataPos numeric vector with x/y starting coordinates each marker should be plotted on the face. Should have equal lenght to the number of columns of data. Default is FIX ME
#' @param dataScale numeric vector of four values containing measured left pupil - right pupil distance, left mouthcorner - right mouthcorner distance, left pupil - left mouthcorner distance, and right pupil - right mouthconrer distance.
#' @param center if TRUE, center x/y values by setting the x/y values to 0 at the first (appropriate, e.g. non NA frame) frame. Default = TRUE
#' @param color character vector of lenght one to define the color for plotting the Marker. Default is "black".
#' @param title Default is no title.
#' @param overplot If TRUE, only the marker plots, but no base graph is plotted. Default is FALSE.
#' @param alpha alpha value / density for marker plots. Default = .99
#' @param rwMeasure Optional. Real world measure of the object diameter used to rescale. Default is the diameter of a glue dot of 8 millimeter.
#'
#' @return nice "Quadratschaedel" (square head) Plot
#'
#' @author Rainer Alexandrowicz, Axel Zinkernagel \email{rainer.alexandrowicz@aau.at, zinkernagel@uni-landau.de}
#'
#' @examples
#' PlotMarkerXHead()
#' 
#' @export
PlotMarkerXHead <- function(data, dataPos, dataScale, center=TRUE, title="", overplot=FALSE, color="black", alpha = .99, rwMeasure = 8) {
  # Error handling
  ...
  # main plot function
  if (! overplot) { # print xHead
    plot(0:1,0:1,xlim=c(-mx,mx),ylim=c(-mx,mx),         # plot area
         type="n",axes=F,main=title,xlab="",ylab="")
    box()
    abline(h=0,col=grey(.8),lty=3)                      # horizontal axis
    abline(v=0,col=grey(.8),lty=3)                      # vertical axis
    ...
    segments(-h0/2 ,-dataScale[3] ,               # mouth line
             h0/2 ,-dataScale[4] , lwd=2 )       # --"--
    ...
    if (shape == 1) rect(x0,y0,x1,y1,border=grey(.7))   # head (rectangle)
    if (shape == 2) oct (x0,y0,x1,y1,border=grey(.7))   # head (octogon)
