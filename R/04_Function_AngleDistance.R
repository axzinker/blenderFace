...
#'
#' Computes the distance and the angle for x/y marker movement. Data is supposed to start at x = 0, y =0, use function CenterCond before.
#' 
...
AngleDistance <- function(x, y, verbose=FALSE) {
  ...
  getDistance <- function(Point1,Point2) {
    # Computes distance between two points in a coordinate system
    # Its a two dimensional distance (not eukledian)
    return(sqrt((Point2[1] - Point1[1])^2 + (Point2[2] - Point1[2])^2))
  }
  
  getAngle <- function(x,y) {
    # Computes the angle between the median of data points and origin of coordinate system
    # Division by (pi/180) necessary to get deg (not rad)
    angle <- NA
